    ...
    <CTable striped hover bordered>
      <CTableHead>
        <CTableRow>
          ...
          <CTableHeaderCell scope="col">SRP</CTableHeaderCell>
        </CTableRow>
      </CTableHead>
      <CTableBody>
        <CTableRow v-for="(product, index) in products" :key="product.productId">
          <CTableHeaderCell scope="row">{{ index + 1 }}</CTableHeaderCell>
          <CTableDataCell>{{ product.sku }}</CTableDataCell>
          <CTableDataCell>{{ product.productName }}</CTableDataCell>
          <CTableDataCell>{{ product.quantityInStock }}</CTableDataCell>
          <CTableDataCell>{{ product.costPrice }}</CTableDataCell>
          <CTableDataCell>{{ product.srpPrice }}</CTableDataCell>
        </CTableRow>
      </CTableBody>
    </CTable>
  </div>
...
onMounted(async () => {
  try {
    const response = await axios.get('http://localhost:5279/api/inventory')
